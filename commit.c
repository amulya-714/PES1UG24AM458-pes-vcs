...
//
// Commit object format (stored as text):
...
int commit_create(const char *message, ObjectID *commit_id_out) {
    ...
    // 3. Try to read current HEAD as parent commit
    ObjectID parent_id;
    ...
    // 7. Return the commit ID if the caller wants it
    if (commit_id_out) *commit_id_out = commit_id;

    // Print confirmation like real git
    char hex[HASH_HEX_SIZE + 1];
